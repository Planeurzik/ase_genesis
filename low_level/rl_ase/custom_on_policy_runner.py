...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        self.alg: PPO | Distillation = alg_class(policy, device=self.device, **self.alg_cfg, multi_gpu_cfg=self.multi_gpu_cfg,
                                                 encoder_discriminator=self.env.encoder_discriminator,
                                                 latent_dim=self.env.latent_dim,
                                                 num_actions=self.env.num_actions,
                                                 enc_disc_nb_updates=self.cfg["enc_disc_nb_updates"],
                                                 enc_disc_batch_size=self.cfg["enc_disc_batch_size"],
                                                 kappa=self.env.kappa,
                                                 w_div=self.cfg["w_div"],
                                                 w_gp=self.cfg["w_gp"],
                                                 dataset=self.cfg["dataset"],
                                                 scales=self.env.obs_scales
                                                 )
    ...
    def save(self, path: str, infos=None):
        ...
        saved_dict = {
            ...
            "infos": infos,
            # Added by Philippe
            "encoder_discriminator_dict": self.env.encoder_discriminator.state_dict()
        }
    ...
    def load(self, path: str, load_optimizer: bool = True):
        ...
        # Added by Philippe
        self.env.encoder_discriminator.load_state_dict(loaded_dict["encoder_discriminator_dict"])

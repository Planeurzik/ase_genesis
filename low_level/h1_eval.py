...
def main():
    ...
    customized_z = torch.tensor([[0.0, 0.0, 0.0, 1.0]], device=device) # z to move forward
    ...
    with torch.no_grad():
        while True:
            ...
            if args.env_real_type:
                ema_alpha = 0.7
                if "ema_tail" not in locals():
                    ema_tail = torch.zeros_like(actions[:,-8:], device=device)
                ema_tail = ema_alpha * ema_tail + (1 - ema_alpha) * actions[:,-8:]
                actions[:,-8:] = ema_tail
